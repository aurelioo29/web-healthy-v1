...
export const EMPTY_LOCALE = { title: "", subtitle: "", body_html: "" };

const CREATE_PATH = "/upload/about-us";
const UPDATE_PATH = "/upload/about-us";
...
const buildImageUrl = (image, imageUrlFromBE) => {
  ...
  const rel = image.includes("/") ? image : `about/${image}`;
...
/** Kirim i18n nested array + fallback flat keys agar kompatibel dengan BE lama/baru */
function appendI18nToFormData(fd, i18nObj) {
  Object.entries(i18nObj).forEach(([loc, vals], idx) => {
    const title = vals?.title ?? "";
    const subtitle = vals?.subtitle ?? "";
    ...
    // flat fallbacks (versi lama)
    fd.append(`title_${loc}`, title);
    fd.append(`subtitle_${loc}`, subtitle);
    fd.append(`body_${loc}`, body_html); // beberapa BE lama pakai body_{loc}
    fd.append(`body_html_${loc}`, body_html); // beberapa BE baru pakai body_html_{loc}
  });
}

/** Normalisasi i18n => { id: {...}, en: {...} } dari berbagai bentuk data awal */
function normalizeI18n(src, initialData) {
  const base = { id: { ...EMPTY_LOCALE }, en: { ...EMPTY_LOCALE } };

  if (Array.isArray(src)) {
    // [{ locale:'id', title, subtitle, body_html }, ...]
    src.forEach((tr) => {
    ...
    base.id = {
      ...
      subtitle:
        src.id?.subtitle ??
        initialData?.subtitle_id ??
        initialData?.i18n_id?.subtitle ??
        "",
      body_html:
        src.id?.body_html ??
    ...
    base.en = {
      ...
      subtitle:
        src.en?.subtitle ??
        initialData?.subtitle_en ??
        initialData?.i18n_en?.subtitle ??
        "",
      body_html:
        src.en?.body_html ??
...
export default function AboutFormModal({
  ...
  useEffect(() => {
    ...
    if (isEdit && initialData) {
      setKeyVal(initialData.key || initialData.section_key || "");
      setPosition(initialData.position || "left");
      setOrderNo(Number(initialData.order_no || 1));
      setIsActive(
        Boolean(
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      fd.append("section_key", keyVal);
      fd.append("position", position);
      fd.append("order_no", String(orderNo || 1));
      fd.append("is_active", String(!!isActive));
      if (imageFile) fd.append("image", imageFile);

      // kirim semua i18n agar kompatibel dengan endpoint create
      appendI18nToFormData(fd, i18n);

      if (isEdit && initialData?.id) {
        // Saat UPDATE, kirim juga field "aktif" agar BE bisa update single-locale
