...
const fmtYMD = (v) => {
  ...
  // Sudah ada pola YYYY-MM-DD di depan? langsung pakai
...
/* =========================
   Row
========================= */
function AboutRow({ item, onEdit, onDelete, onPreview }) {
...
export default function AboutTable() {
  ...
  const fetchList = async ({
    ...
    setErr("");
    try {
      const { data } = await api.get("/upload/about-us", {
        params: {
          page: pageArg,
          size: sizeArg,
          ...(searchArg ? { search: searchArg } : {}),
        },
        headers: { "Cache-Control": "no-cache" }, // opsional
      });

      // data.data bisa berupa array ATAU object
      const payload = data?.data;

      // Ambil sections
      const sections = Array.isArray(payload)
        ? payload
        : payload?.sections || payload?.items || payload?.rows || [];

      // Hitung meta kalau payload array (tanpa pagination dari BE)
      const total = Array.isArray(payload)
        ? payload.length
        : payload?.totalSections ?? payload?.total ?? sections.length;
      ...
      const mapped = (sections || []).map((s, i) => {
        ...
        // ⬇️ fallback untuk berbagai nama field tanggal dari BE
        const created =
          s.created_at ??
          s.createdAt ??
          s.created ??
        ...
        return {
          ...
          date: fmtYMD(created), // ⬅️ sekarang aman
  ...
  const onDelete = async (item) => {
    if (!confirm(`Delete section "${item.key}"? This cannot be undone.`))
      return;
    try {
      await api.delete(`/upload/about-us/${item.id}`);
      setPage(1);
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-slate-50 text-slate-600">
            <tr>
              ...
              <th className="py-3 px-3 text-left font-semibold">Section</th>
