...
export default function InvestorCategoriesTable() {
  ...
  const fetchList = async (
    ...
    setErr("");
    try {
      const { data } = await api.get("/upload/category-investors", {
        params: {
          page: pageArg,
          size: sizeArg,
      ...
      // ==== ROBUST UNWRAP: dukung berbagai bentuk payload ====
      const box = data?.data || {};
      const list =
        box.categories ||
        box.category_investors || // fallback ke versi lama
        box.items ||
        box.rows ||
        [];
      ...
      if (e?.response?.status === 404) {
        // list kosong seharusnya 200, tapi kalau BE kirim 404 ya kita handle
        setRows([]);
  ...
  const onDelete = async (item) => {
    if (!confirm(`Delete category "${item.name}"? This cannot be undone.`))
      return;
    try {
      await api.delete(`/upload/category-investors/${item.id}`);
      setPage(1);
