...
const CREATE_PATH = "/upload/articles";
const UPDATE_PATH = "/upload/articles";
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const arr = data?.data?.category_articles || [];
        setCategories(arr.map((c) => ({ id: c.id, name: c.name })));
      } catch {
        setCategories([]);
      }
    };
      ...
      <div className="w-full max-w-3xl rounded-2xl bg-white p-6 shadow-xl max-h-[85vh] overflow-y-auto">
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div>
              ...
              <select
                ...
                className={`mt-1 w-full rounded-lg ring-1 px-3 py-2 text-sm outline-none focus:ring-sky-400 ${
                  errors.category_id ? "ring-rose-300" : "ring-slate-200"
                }`}
                required
              >
          ...
          <div>
            ...
            >
              <CsrEditor value={content} onChange={setContent} />
            </div>
